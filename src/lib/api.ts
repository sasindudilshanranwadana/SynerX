  ...
  try {
    ...
    const uploadData = await new Promise<any>((resolve, reject) => {
      ...
      xhr.addEventListener('abort', () => {
        const error: any = new Error('Upload cancelled');
...
export const clearCompletedRunPodJobs = async (): Promise<{ success: boolean; cleared?: number }> => {
  try {
    const response = await fetch(`${RUNPOD_API_BASE}/jobs/clear-completed`, { method: 'POST' });
    if (!response.ok) {
...
export const shutdownAllRunPodJobs = async (): Promise<{ success: boolean; shutdown?: number }> => {
  try {
    const response = await fetch(`${RUNPOD_API_BASE}/jobs/shutdown`, { method: 'POST' });
    if (!response.ok) {
      throw new Error('Failed to shutdown all jobs');
    }
    return await response.json();
  } catch (error) {
...
export const shutdownSpecificRunPodJob = async (jobId: string): Promise<{ success: boolean; job_id?: string }> => {
  try {
    const response = await fetch(`${RUNPOD_API_BASE}/jobs/shutdown/${jobId}`, { method: 'POST' });
    if (!response.ok) {
      throw new Error(`Failed to shutdown job ${jobId}`);
    }
    return await response.json();
  } catch (error) {
